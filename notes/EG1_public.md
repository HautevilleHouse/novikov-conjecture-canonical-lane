...
In-paper anchor: `paper/NOVIKOV_CONJECTURE_PREPRINT.md` (`NOV_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving homotopy invariance of higher signatures through an admissible assembly-signature closure architecture`.
...
- projected core: the response sector controlled by `kappa_signature`.
...
`NOV_G1` closes when `kappa_signature` satisfies the response-floor requirement: projected signature response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_NOV`.
...
Payload: verify that all quantities used by `kappa_signature` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `kappa_signature` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_signature`, then `NOV_G1` supplies the projected/protected-core input to `M_NOV`.

## Current Instantiation

- gate: `NOV_G1`
- artifact key: `kappa_signature`
