...
In-paper anchor: `paper/NOVIKOV_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving homotopy invariance of higher signatures through an admissible assembly-signature closure architecture`.
...
- endpoint lock: `signature_transfer`.
...
2. the endpoint-transfer lock `signature_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_NOV` remains positive.
...
Payload: verify `signature_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_NOV` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `signature_transfer`
